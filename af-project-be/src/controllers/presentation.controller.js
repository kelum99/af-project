...
exports.createpresentationEvaluation = async (req, res) => {
  try {
    ...
    if (savedpresentationEvaluation) {
      res
        .status(201)
        .send({ message: "success!", data: savedpresentationEvaluation });
    } else {
      res
        .status(400)
        .send({ message: "failed!", data: savedpresentationEvaluation });
    }
  } catch (e) {
    console.log("error", e);
...
exports.getpresentationEvaluation = async (req, res) => {
  try {
    const PresentationEvaluation = await presentationEvaluation.findById(
      req.params.id
    );
...
exports.updatepresentationEvaluation = async (req, res) => {
  try {
    const data = req.body;
    if (data) {
      const updatepresentationEvaluation = await presentationEvaluation.updateOne(
        { _id: req.params.id },
        { ...data }
      );
      ...
      res
        .status(200)
        .send({ message: "success", data: updatepresentationEvaluation });
    } else {
...
exports.deletepresentationEvaluation = async (req, res) => {
  try {
    const deletepresentationEvaluation = await presentationEvaluation.findByIdAndDelete(
      req.params.id
    );
